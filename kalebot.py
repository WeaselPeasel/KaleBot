...
async def on_message(message):
	...
	if message.content.startswith(command_prefix+feature_request_command):
		split_message = message.content.split(command_prefix+feature_request_command, 1)
		try:
			feature = split_message[1]
		except IndexError as e:
			kalebot_logger.error("A user did not provide a feature: " + e)
			await message.channel.send(embed=get_embed(title="ERROR", body="No feature was provided"))
		else:
			fp = open(FEATURES_FILE, "a")
			fp.write(feature)
			fp.close()
			await message.channel.send(embed=get_embed(title="Success", body="Your feature request has been added!"))
